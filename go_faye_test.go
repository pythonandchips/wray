...
func TestSubscribe(t *testing.T) {
  Convey("subscribe to a channel when unconnected", t, func() {
    ...
    Given(func(){ response = Response{id: "1", channel: "/meta/handshake", successful: true, clientId: "client4", supportedConnectionTypes: []string{"long-polling"}} })
    ...
    Given(func(){ subscriptionParams = map[string]interface{}{"channel": "/meta/subscribe", "clientId": response.clientId, "subscription": "/foo/*", "id": "1"} })
    ...
    Convey("add the subscription to the client", func(){
      ...
      Then(func() { So(fayeClient.subscriptions[0].callback, ShouldEqual, subscriptionPromise.subscription.callback) })
    })
...
func TestPerformHandshake(t *testing.T) {
  Convey("successful handshake with server", t, func() {
    ...
    var response Response
    Given(func(){ handshakeParams = map[string]interface{}{"channel": "/meta/handshake",
                                                           "version": "1.0",
  ...
  Convey("unsuccessful handshake with server", t, func() {
    ...
    Given(func(){ response = Response{id: "1", channel: "/meta/handshake", successful: false, clientId: "client4", supportedConnectionTypes: []string{"long-polling"}} })
    Given(func(){ fakeHttpTransport = &FakeHttpTransport{usable: true, response: response} })
    Given(func(){ registeredTransports = []Transport{fakeHttpTransport} })
    Given(func(){ fayeClient = BuildFayeClient().Client() })
    When(func(){ fayeClient.handshake() })
    Then(func(){ So(fayeClient.state, ShouldEqual, UNCONNECTED) })
    Then(func(){ So(fayeClient.schedular.delay(), ShouldEqual, 1 * time.Minute) })
  })
  ...
  Convey("when server does not support available transports", t, func() {
    ...
    Given(func(){ response = Response{id: "1", channel: "/meta/handshake", successful: true, clientId: "client4", supportedConnectionTypes: []string{"web-socket"}} })
    Given(func(){ fakeHttpTransport = &FakeHttpTransport{usable: true, response: response} })
    Given(func(){ registeredTransports = []Transport{fakeHttpTransport} })
    Given(func(){ fayeClient = BuildFayeClient().Client() })
    Then(func(){ So(func(){ fayeClient.handshake()}, ShouldPanicWith, "Server does not support any available transports. Supported transports: web-socket") })
  })
}
...
func TestHandleResponse(t *testing.T){
  Convey("when server does not support available transports", t, func() {
    ...
    Given(func(){ subscriptions = []Subscription{ Subscription{"/foo/bar", func(message Message){ firstMessages = append(firstMessages, message) }},
    ...
    Given(func(){ messages = []Message{ Message{Channel: "/foo/bar", Id: "1", Data: firstParams },
...
func TestPublish(t *testing.T){
  Convey("publish message to server", t, func() {
    ...
    Then(func(){ So(fakeHttpTransport.sentParams, ShouldResemble, map[string]interface{}{"channel": "/foo", "data": data, "clientId": fayeClient.clientId})  })
